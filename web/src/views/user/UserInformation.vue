  ...
  <div class='userinformation'>
    ...
    <div class="content">
      <div class="p-shadow-1">
        <Toolbar>
          <template #left>
            <Button icon="pi pi-user-edit" label="Update" @click="update()"
              :disabled="!isValid || !isDifferent" />
          </template>
          <template #right>
            <Button icon="pi pi-undo" label="Restore" @click="restore()"
                :disabled="!isDifferent" class="p-button-warning" />
          </template>
        </Toolbar>
        <DataTable :value="tableData" class="editable-cells-table"
...
export default {
  ...
  computed: {
    ...
    isNameDifferent() { return this.name != this.data?.name },
    isLastnameValid() {
      return [
        this.lastname.length > 0,
      ].every(e => e)
    },
    isLastnameDifferent() { return this.lastname != this.data?.lastname },
    isEmailValid() {
      return [
        this.email.length > 0,
      ].every(e => e)
    },
    isEmailDifferent() { return this.email != this.data?.email },
    isAddressValid() {
      return [
        this.address.length > 0,
      ].every(e => e)
    },
    isAddressDifferent() { return this.address != this.data?.address },
    isWorkphoneValid() {
      return [
        this.workphone.length > 0,
      ].every(e => e)
    },
    isWorkphoneDifferent() { return this.workphone != this.data?.workphone },
    isMobilephoneValid() {
      return [
        this.mobilephone.length > 0,
      ].every(e => e)
    },
    isMobilephoneDifferent() { return this.mobilephone != this.data?.mobilephone },
    isValid() {
      return [
        this.isNameValid,
  ...
  methods: {
    onCellEditComplete(event) {
      switch (event.index) {
        case 0: this.name = event.data.value; break;
        case 1: this.lastname = event.data.value; break;
        case 2: this.email = event.data.value; break;
        case 3: this.address = event.data.value; break;
        case 4: this.workphone = event.data.value; break;
        case 5: this.mobilephone = event.data.value; break;
      }
    },
    hasError(field) {
